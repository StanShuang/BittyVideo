...
class AutoPlayPageChangeListener(
    private val viewPage: ViewPager2,
    ...
    override fun onPageScrollStateChanged(state: Int) {
        ...
        if (isPageSelected && state == ViewPager2.SCROLL_STATE_IDLE) {
            playVideo()
    ...
    private fun playVideo() {
        ...
        for (childIndex in 0 until viewPage.childCount) {
            val view = viewPage.getChildAt(childIndex)
            if (view != null && view is RecyclerView) {
                val layoutManager = view.layoutManager
                //返回offscreenPageLimit(两侧保留的屏幕页数) * 2 + 1（当前页数）。offscreenPageLimit 设置1，则返回值为3。
                ...
                for (index in 0 until childCount) {
                    if (layoutManager?.getChildAt(index) != null && layoutManager.getChildAt(index)
                        ...
                        if (isPlayerVisible && player.isVisible && (player.currentPlayer.currentState == GSYBaseVideoPlayer.CURRENT_STATE_NORMAL || player.currentPlayer.currentState == GSYBaseVideoPlayer.CURRENT_STATE_ERROR)) {
